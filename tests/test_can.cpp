...
TEST_SUITE("CAN Functions")
{
    ...
    {
        uint8_t buf[64];
        float input = 12.345f; // Make sure this *1000 fits in int16_t
        can_setSignal<int16_t>(buf, input, 0, 16, true, 0.001, 0);
        float output = can_getSignal<int16_t>(buf, 0, 16, true, 0.001, 0);
        CHECK(fabs(input - output) < 0.001);
    }
}